...
6. **Atualizar Swagger e Postman**: após qualquer mudança que afete endpoints (novo handler, rota alterada, payload ou response modificado), executar `make swagger` para regenerar o OpenAPI e atualizar o `postman_collection.json` correspondente, que está em `/docs`. Incluir ambos no mesmo commit da mudança.
